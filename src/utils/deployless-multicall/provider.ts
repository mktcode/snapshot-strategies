...
import type { Call, CallOverrides } from './call';
import {
  all as callAll,
...
import { getMulticall, getMulticall2, getMulticall3 } from './multicall';
...
class Provider {
   ...
   */
  async all<T>(calls: Call[], overrides?: CallOverrides, forceDeployless?: boolean): Promise<T[]> {
    if (!this.#provider) {
      throw Error('Provider should be initialized before use.');
    }
    const multicall = this.#getContract('BASIC', overrides?.blockTag);
    const provider = this.#provider;
    return await callAll<T>(provider, multicall, calls, overrides, forceDeployless);
  }
